...
namespace LightningForge.Arcade.Net
{
    ...
    public class ChessSession : MonoBehaviour
    {
        ...
        public void Leave()
        {
            // Detach before shutting down. Shutdown takes a moment, IsConnected goes false
            // as soon as it starts, and the lobby offers Play Online again the instant it
            ...
            // Kept so the next connect can wait for it. Two runners must never be live at
            // once: they authenticate as the same Photon user, and the server resolves that
            // by dropping one of them with a ServerLogic disconnect.
            if (leaving != null) shuttingDown = leaving.Shutdown();
        }
